...
# Dictionary to map encoded IDs to actual file IDs
file_store = {}
...
async def start(bot, message):
    if len(message.command) > 1:
        ...
        try:
            ...
            # Look up the file in the file_store
            if file_id in file_store:
                file_metadata = file_store[file_id]
                await bot.send_document(
                    chat_id=message.chat.id,
                    document=file_id,
                    caption=file_metadata.get("caption", "Here is your file!")
                )
            else:
...
async def set_poster(_, message: Message):
    if not message.from_user:
        await message.reply_text(
            "Could not identify the user. Make sure you are not sending from a channel or as an anonymous admin.",
            quote=True,
        )
        return
    ...
    episode_data[user_id]["poster_msg_id"] = sent_poster.id
...
async def process_quality(message, user_id, anime_details, episodes, episode_key):
    ...
    await bot.edit_message_text(
        chat_id=TARGET_CHANNEL,
        message_id=store_msg.id,
        text=f"{quality} file uploaded to file store channel ✅",
    )

    # Store metadata for file retrieval
    file_id = sent_message.document.file_id
    file_store[file_id] = {
        "caption": f"{anime_details['name']} - Episode {anime_details['episode']} ({quality})",
    }
...
async def update_poster_buttons(message, user_id, anime_details, episodes, episode_key):
    ...
    if poster_msg_id:
        ...
        episodes[episode_key]["poster_msg_id"] = sent_poster.id
